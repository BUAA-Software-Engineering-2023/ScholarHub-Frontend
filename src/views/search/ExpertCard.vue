			...
			<div class="authorGrid">
				<a-space direction="vertical">
					<a-space direction="horizontal">
						<a-typography-text keyboard>H-index:{{item.cited_by_count}}</a-typography-text>
						<a-typography-text keyboard>引用数:{{item.cited_by_count}}</a-typography-text>
					</a-space>
					<a-typography-text>{{item.last_known_institution.display_name}}</a-typography-text>
					<a-space direction="horizontal">
					<a-typography-text :level="5">研究领域：</a-typography-text>
					<a-typography-text :level="5"  v-for="(author, index) in item.x_concepts"   >
						<a-typography-text v-if="index<3">{{author.display_name}}, &nbsp</a-typography-text>
					</a-typography-text>
					</a-space>
				</a-space>
			</div>
