      ...
      <a-layout style="width: 80%">
        <a-layout-header theme="light" :style="headerStyle" >
          ...
          <div class="publisher-info">
            ...
            <div class="publisher-details" style="display: flex ">
...
<!--            <a-list item-layout="vertical" size="small" :split="true" :data-source="associated_publishers" v-show="showSide">-->
...
const series = ref([]);
const years = ref([]);
const associated_publishers = ref([]);
const related_publishers = ref([]);
const PublisherId ="https://openalex.org/"+route.params.publisherId
...
onMounted(async ()=>{
  pflag.value = false;
  const result =  await Search.publisher_detail(PublisherId)
  if (result.data.success){
    const response = result.data.data
    console.log(response)
    publisher.value = result.data.data
    publishername.value = publisher.value.display_name
    works_count.value = publisher.value.works_count
    cited_by_count.value = publisher.value.cited_by_count
    h_index.value = publisher.value.summary_stats.h_index
    description.value = publisher.value.description
    related_publishers.value = publisher.value.related_publishers
    avatarsrc.value = publisher.value.image_thumbnail_url
    homepage_url.value = publisher.value.homepage_url
    ...
    // const associated_publishers_list = publisher.value.related_publishers

    // for (let i = 0 ;i<associated_publishers_list.length;i++){
    //   const url = associated_publishers_list[i].id
    //   const parts = url.split('/');
    //   const publisherId = parts[parts.length - 1];
    //   const publisherActions = [
    //     { text: "score:  "+associated_publishers_list[i].score.toString() },
    //   ];
    //   associated_publishers.value.push({
    //     href: "/client/publisher/"+ publisherId,
    //     title: associated_publishers_list[i].display_name,
...
.publisher-info .publisher-details {
  align-items: center;
  flex-grow: 1;
}

.publisher-info .h-index {
  text-align: right;
}
